...
class JackAnalyzer:
    ...
    def analyze(self):
        ...
        for target in self.targets:
            ...
            # Set up XML and VM output files
            xml_output_file = os.path.join(target_dir, os.path.basename(target).replace('.jack', '.xml'))
            vm_output_file = os.path.join(target_dir, os.path.basename(target).replace('.jack', '.vm'))

            if os.path.exists(xml_output_file):
                os.remove(xml_output_file)

            compiler = CompilationEngine(tokenizer, xml_output_file, vm_output_file)
            compiler.compile_class()
            compiler.save_output()  # save XML output
            compiler.close_vm_output()  # close VM output after writing

            print(f'{target} parsed and compiled successfully')
